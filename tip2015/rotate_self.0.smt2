(set-logic HORN)
(declare-datatypes ((Nat_1 0)) (((Z_0 ) (S_0 (unS_0 Nat_1)))))
(declare-fun diseqNat_0 (Nat_1 Nat_1) Bool)
(declare-fun unS_1 (Nat_1 Nat_1) Bool)
(declare-fun isZ_0 (Nat_1) Bool)
(declare-fun isS_0 (Nat_1) Bool)
(assert (forall ((x_19 Nat_1) (x_18 Nat_1))
	(=> (= x_19 (S_0 x_18))
	    (unS_1 x_18 x_19))))
(assert (isZ_0 Z_0))
(assert (forall ((x_20 Nat_1))
	(isS_0 (S_0 x_20))))
(assert (forall ((x_21 Nat_1))
	(diseqNat_0 Z_0 (S_0 x_21))))
(assert (forall ((x_22 Nat_1))
	(diseqNat_0 (S_0 x_22) Z_0)))
(assert (forall ((x_23 Nat_1) (x_24 Nat_1))
	(=> (diseqNat_0 x_23 x_24)
	    (diseqNat_0 (S_0 x_23) (S_0 x_24)))))
(declare-fun add_0 (Nat_1 Nat_1 Nat_1) Bool)
(declare-fun minus_0 (Nat_1 Nat_1 Nat_1) Bool)
(declare-fun le_0 (Nat_1 Nat_1) Bool)
(declare-fun ge_0 (Nat_1 Nat_1) Bool)
(declare-fun lt_0 (Nat_1 Nat_1) Bool)
(declare-fun gt_0 (Nat_1 Nat_1) Bool)
(declare-fun mult_0 (Nat_1 Nat_1 Nat_1) Bool)
(declare-fun div_0 (Nat_1 Nat_1 Nat_1) Bool)
(declare-fun mod_0 (Nat_1 Nat_1 Nat_1) Bool)
(assert (forall ((y_2 Nat_1))
	(add_0 y_2 Z_0 y_2)))
(assert (forall ((x_16 Nat_1) (y_2 Nat_1) (r_0 Nat_1))
	(=> (add_0 r_0 x_16 y_2)
	    (add_0 (S_0 r_0) (S_0 x_16) y_2))))
(assert (forall ((y_2 Nat_1))
	(minus_0 Z_0 Z_0 y_2)))
(assert (forall ((x_16 Nat_1) (y_2 Nat_1) (r_0 Nat_1))
	(=> (minus_0 r_0 x_16 y_2)
	    (minus_0 (S_0 r_0) (S_0 x_16) y_2))))
(assert (forall ((y_2 Nat_1))
	(le_0 Z_0 y_2)))
(assert (forall ((x_16 Nat_1) (y_2 Nat_1))
	(=> (le_0 x_16 y_2)
	    (le_0 (S_0 x_16) (S_0 y_2)))))
(assert (forall ((y_2 Nat_1))
	(ge_0 y_2 Z_0)))
(assert (forall ((x_16 Nat_1) (y_2 Nat_1))
	(=> (ge_0 x_16 y_2)
	    (ge_0 (S_0 x_16) (S_0 y_2)))))
(assert (forall ((y_2 Nat_1))
	(lt_0 Z_0 (S_0 y_2))))
(assert (forall ((x_16 Nat_1) (y_2 Nat_1))
	(=> (lt_0 x_16 y_2)
	    (lt_0 (S_0 x_16) (S_0 y_2)))))
(assert (forall ((y_2 Nat_1))
	(gt_0 (S_0 y_2) Z_0)))
(assert (forall ((x_16 Nat_1) (y_2 Nat_1))
	(=> (gt_0 x_16 y_2)
	    (gt_0 (S_0 x_16) (S_0 y_2)))))
(assert (forall ((y_2 Nat_1))
	(mult_0 Z_0 Z_0 y_2)))
(assert (forall ((x_16 Nat_1) (y_2 Nat_1) (r_0 Nat_1) (z_3 Nat_1))
	(=>	(and (mult_0 r_0 x_16 y_2)
			(add_0 z_3 r_0 y_2))
		(mult_0 z_3 (S_0 x_16) y_2))))
(assert (forall ((x_16 Nat_1) (y_2 Nat_1))
	(=> (lt_0 x_16 y_2)
	    (div_0 Z_0 x_16 y_2))))
(assert (forall ((x_16 Nat_1) (y_2 Nat_1) (r_0 Nat_1) (z_3 Nat_1))
	(=>	(and (ge_0 x_16 y_2)
			(minus_0 z_3 x_16 y_2)
			(div_0 r_0 z_3 y_2))
		(div_0 (S_0 r_0) x_16 y_2))))
(assert (forall ((x_16 Nat_1) (y_2 Nat_1))
	(=> (lt_0 x_16 y_2)
	    (mod_0 x_16 x_16 y_2))))
(assert (forall ((x_16 Nat_1) (y_2 Nat_1) (r_0 Nat_1) (z_3 Nat_1))
	(=>	(and (ge_0 x_16 y_2)
			(minus_0 z_3 x_16 y_2)
			(mod_0 r_0 z_3 y_2))
		(mod_0 r_0 x_16 y_2))))
(declare-datatypes ((list_0 0)) (((nil_0 ) (cons_0 (head_0 Nat_1) (tail_0 list_0)))))
(declare-fun diseqlist_0 (list_0 list_0) Bool)
(declare-fun head_1 (Nat_1 list_0) Bool)
(declare-fun tail_1 (list_0 list_0) Bool)
(declare-fun isnil_0 (list_0) Bool)
(declare-fun iscons_0 (list_0) Bool)
(assert (forall ((x_28 list_0) (x_26 Nat_1) (x_27 list_0))
	(=> (= x_28 (cons_0 x_26 x_27))
	    (head_1 x_26 x_28))))
(assert (forall ((x_28 list_0) (x_26 Nat_1) (x_27 list_0))
	(=> (= x_28 (cons_0 x_26 x_27))
	    (tail_1 x_27 x_28))))
(assert (isnil_0 nil_0))
(assert (forall ((x_29 Nat_1) (x_30 list_0))
	(iscons_0 (cons_0 x_29 x_30))))
(assert (forall ((x_31 Nat_1) (x_32 list_0))
	(diseqlist_0 nil_0 (cons_0 x_31 x_32))))
(assert (forall ((x_33 Nat_1) (x_34 list_0))
	(diseqlist_0 (cons_0 x_33 x_34) nil_0)))
(assert (forall ((x_35 Nat_1) (x_36 list_0) (x_37 Nat_1) (x_38 list_0))
	(=> (diseqNat_0 x_35 x_37)
	    (diseqlist_0 (cons_0 x_35 x_36) (cons_0 x_37 x_38)))))
(assert (forall ((x_35 Nat_1) (x_36 list_0) (x_37 Nat_1) (x_38 list_0))
	(=> (diseqlist_0 x_36 x_38)
	    (diseqlist_0 (cons_0 x_35 x_36) (cons_0 x_37 x_38)))))
(declare-datatypes ((Nat_0 0)) (((zero_0 ) (succ_0 (p_0 Nat_0)))))
(declare-fun diseqNat_1 (Nat_0 Nat_0) Bool)
(declare-fun p_1 (Nat_0 Nat_0) Bool)
(declare-fun iszero_0 (Nat_0) Bool)
(declare-fun issucc_0 (Nat_0) Bool)
(assert (forall ((x_41 Nat_0) (x_40 Nat_0))
	(=> (= x_41 (succ_0 x_40))
	    (p_1 x_40 x_41))))
(assert (iszero_0 zero_0))
(assert (forall ((x_42 Nat_0))
	(issucc_0 (succ_0 x_42))))
(assert (forall ((x_43 Nat_0))
	(diseqNat_1 zero_0 (succ_0 x_43))))
(assert (forall ((x_44 Nat_0))
	(diseqNat_1 (succ_0 x_44) zero_0)))
(assert (forall ((x_45 Nat_0) (x_46 Nat_0))
	(=> (diseqNat_1 x_45 x_46)
	    (diseqNat_1 (succ_0 x_45) (succ_0 x_46)))))
(declare-fun x_0 (list_0 list_0 list_0) Bool)
(assert (forall ((x_1 list_0) (y_0 list_0) (z_0 Nat_1) (xs_0 list_0) (x_3 list_0) (x_4 list_0))
	(=>	(and (= x_3 (cons_0 z_0 x_4))
			(x_0 x_4 xs_0 y_0)
			(= x_1 (cons_0 z_0 xs_0)))
		(x_0 x_3 x_1 y_0))))
(assert (forall ((x_1 list_0) (y_0 list_0) (x_5 list_0))
	(=>	(and (= x_5 y_0)
			(= x_1 nil_0))
		(x_0 x_5 x_1 y_0))))
(declare-fun rotate_0 (list_0 Nat_0 list_0) Bool)
(assert (forall ((x_2 Nat_0) (y_1 list_0) (z_1 Nat_0) (z_2 Nat_1) (xs_1 list_0) (x_6 list_0) (x_7 list_0) (x_8 list_0))
	(=>	(and (= x_6 x_8)
			(x_0 x_7 xs_1 (cons_0 z_2 nil_0))
			(rotate_0 x_8 z_1 x_7)
			(= x_2 (succ_0 z_1))
			(= y_1 (cons_0 z_2 xs_1)))
		(rotate_0 x_6 x_2 y_1))))
(assert (forall ((x_2 Nat_0) (y_1 list_0) (z_1 Nat_0) (x_9 list_0))
	(=>	(and (= x_9 nil_0)
			(= x_2 (succ_0 z_1))
			(= y_1 nil_0))
		(rotate_0 x_9 x_2 y_1))))
(assert (forall ((x_2 Nat_0) (y_1 list_0) (x_10 list_0))
	(=>	(and (= x_10 y_1)
			(= x_2 zero_0))
		(rotate_0 x_10 x_2 y_1))))
(assert (forall ((n_0 Nat_0) (xs_2 list_0) (x_11 list_0) (x_12 list_0) (x_13 list_0) (x_14 list_0) (x_15 list_0))
	(=>	(and (diseqlist_0 x_12 x_15)
			(x_0 x_11 xs_2 xs_2)
			(rotate_0 x_12 n_0 x_11)
			(rotate_0 x_13 n_0 xs_2)
			(rotate_0 x_14 n_0 xs_2)
			(x_0 x_15 x_13 x_14))
		false)))
(check-sat)
