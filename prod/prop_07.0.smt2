(set-logic HORN)
(declare-datatypes ((Nat_1 0)) (((Z_1 ) (S_1 (unS_0 Nat_1)))))
(declare-fun diseqNat_0 (Nat_1 Nat_1) Bool)
(declare-fun unS_1 (Nat_1 Nat_1) Bool)
(declare-fun isZ_0 (Nat_1) Bool)
(declare-fun isS_0 (Nat_1) Bool)
(assert (forall ((x_22 Nat_1) (x_21 Nat_1))
	(=> (= x_22 (S_1 x_21))
	    (unS_1 x_21 x_22))))
(assert (isZ_0 Z_1))
(assert (forall ((x_23 Nat_1))
	(isS_0 (S_1 x_23))))
(assert (forall ((x_24 Nat_1))
	(diseqNat_0 Z_1 (S_1 x_24))))
(assert (forall ((x_25 Nat_1))
	(diseqNat_0 (S_1 x_25) Z_1)))
(assert (forall ((x_26 Nat_1) (x_27 Nat_1))
	(=> (diseqNat_0 x_26 x_27)
	    (diseqNat_0 (S_1 x_26) (S_1 x_27)))))
(declare-fun add_0 (Nat_1 Nat_1 Nat_1) Bool)
(declare-fun minus_0 (Nat_1 Nat_1 Nat_1) Bool)
(declare-fun le_0 (Nat_1 Nat_1) Bool)
(declare-fun ge_0 (Nat_1 Nat_1) Bool)
(declare-fun lt_0 (Nat_1 Nat_1) Bool)
(declare-fun gt_0 (Nat_1 Nat_1) Bool)
(declare-fun mult_0 (Nat_1 Nat_1 Nat_1) Bool)
(declare-fun div_0 (Nat_1 Nat_1 Nat_1) Bool)
(declare-fun mod_0 (Nat_1 Nat_1 Nat_1) Bool)
(assert (forall ((y_4 Nat_1))
	(add_0 y_4 Z_1 y_4)))
(assert (forall ((x_19 Nat_1) (y_4 Nat_1) (r_0 Nat_1))
	(=> (add_0 r_0 x_19 y_4)
	    (add_0 (S_1 r_0) (S_1 x_19) y_4))))
(assert (forall ((y_4 Nat_1))
	(minus_0 Z_1 Z_1 y_4)))
(assert (forall ((x_19 Nat_1) (y_4 Nat_1) (r_0 Nat_1))
	(=> (minus_0 r_0 x_19 y_4)
	    (minus_0 (S_1 r_0) (S_1 x_19) y_4))))
(assert (forall ((y_4 Nat_1))
	(le_0 Z_1 y_4)))
(assert (forall ((x_19 Nat_1) (y_4 Nat_1))
	(=> (le_0 x_19 y_4)
	    (le_0 (S_1 x_19) (S_1 y_4)))))
(assert (forall ((y_4 Nat_1))
	(ge_0 y_4 Z_1)))
(assert (forall ((x_19 Nat_1) (y_4 Nat_1))
	(=> (ge_0 x_19 y_4)
	    (ge_0 (S_1 x_19) (S_1 y_4)))))
(assert (forall ((y_4 Nat_1))
	(lt_0 Z_1 (S_1 y_4))))
(assert (forall ((x_19 Nat_1) (y_4 Nat_1))
	(=> (lt_0 x_19 y_4)
	    (lt_0 (S_1 x_19) (S_1 y_4)))))
(assert (forall ((y_4 Nat_1))
	(gt_0 (S_1 y_4) Z_1)))
(assert (forall ((x_19 Nat_1) (y_4 Nat_1))
	(=> (gt_0 x_19 y_4)
	    (gt_0 (S_1 x_19) (S_1 y_4)))))
(assert (forall ((y_4 Nat_1))
	(mult_0 Z_1 Z_1 y_4)))
(assert (forall ((x_19 Nat_1) (y_4 Nat_1) (r_0 Nat_1) (z_2 Nat_1))
	(=>	(and (mult_0 r_0 x_19 y_4)
			(add_0 z_2 r_0 y_4))
		(mult_0 z_2 (S_1 x_19) y_4))))
(assert (forall ((x_19 Nat_1) (y_4 Nat_1))
	(=> (lt_0 x_19 y_4)
	    (div_0 Z_1 x_19 y_4))))
(assert (forall ((x_19 Nat_1) (y_4 Nat_1) (r_0 Nat_1) (z_2 Nat_1))
	(=>	(and (ge_0 x_19 y_4)
			(minus_0 z_2 x_19 y_4)
			(div_0 r_0 z_2 y_4))
		(div_0 (S_1 r_0) x_19 y_4))))
(assert (forall ((x_19 Nat_1) (y_4 Nat_1))
	(=> (lt_0 x_19 y_4)
	    (mod_0 x_19 x_19 y_4))))
(assert (forall ((x_19 Nat_1) (y_4 Nat_1) (r_0 Nat_1) (z_2 Nat_1))
	(=>	(and (ge_0 x_19 y_4)
			(minus_0 z_2 x_19 y_4)
			(mod_0 r_0 z_2 y_4))
		(mod_0 r_0 x_19 y_4))))
(declare-datatypes ((list_0 0)) (((nil_0 ) (cons_0 (head_0 Nat_1) (tail_0 list_0)))))
(declare-fun diseqlist_0 (list_0 list_0) Bool)
(declare-fun head_1 (Nat_1 list_0) Bool)
(declare-fun tail_1 (list_0 list_0) Bool)
(declare-fun isnil_0 (list_0) Bool)
(declare-fun iscons_0 (list_0) Bool)
(assert (forall ((x_31 list_0) (x_29 Nat_1) (x_30 list_0))
	(=> (= x_31 (cons_0 x_29 x_30))
	    (head_1 x_29 x_31))))
(assert (forall ((x_31 list_0) (x_29 Nat_1) (x_30 list_0))
	(=> (= x_31 (cons_0 x_29 x_30))
	    (tail_1 x_30 x_31))))
(assert (isnil_0 nil_0))
(assert (forall ((x_32 Nat_1) (x_33 list_0))
	(iscons_0 (cons_0 x_32 x_33))))
(assert (forall ((x_34 Nat_1) (x_35 list_0))
	(diseqlist_0 nil_0 (cons_0 x_34 x_35))))
(assert (forall ((x_36 Nat_1) (x_37 list_0))
	(diseqlist_0 (cons_0 x_36 x_37) nil_0)))
(assert (forall ((x_38 Nat_1) (x_39 list_0) (x_40 Nat_1) (x_41 list_0))
	(=> (diseqNat_0 x_38 x_40)
	    (diseqlist_0 (cons_0 x_38 x_39) (cons_0 x_40 x_41)))))
(assert (forall ((x_38 Nat_1) (x_39 list_0) (x_40 Nat_1) (x_41 list_0))
	(=> (diseqlist_0 x_39 x_41)
	    (diseqlist_0 (cons_0 x_38 x_39) (cons_0 x_40 x_41)))))
(declare-datatypes ((Nat_0 0)) (((Z_0 ) (S_0 (projS_0 Nat_0)))))
(declare-fun diseqNat_1 (Nat_0 Nat_0) Bool)
(declare-fun projS_1 (Nat_0 Nat_0) Bool)
(declare-fun isZ_1 (Nat_0) Bool)
(declare-fun isS_1 (Nat_0) Bool)
(assert (forall ((x_44 Nat_0) (x_43 Nat_0))
	(=> (= x_44 (S_0 x_43))
	    (projS_1 x_43 x_44))))
(assert (isZ_1 Z_0))
(assert (forall ((x_45 Nat_0))
	(isS_1 (S_0 x_45))))
(assert (forall ((x_46 Nat_0))
	(diseqNat_1 Z_0 (S_0 x_46))))
(assert (forall ((x_47 Nat_0))
	(diseqNat_1 (S_0 x_47) Z_0)))
(assert (forall ((x_48 Nat_0) (x_49 Nat_0))
	(=> (diseqNat_1 x_48 x_49)
	    (diseqNat_1 (S_0 x_48) (S_0 x_49)))))
(declare-fun qrev_0 (list_0 list_0 list_0) Bool)
(assert (forall ((x_0 list_0) (y_0 list_0) (z_0 Nat_1) (xs_0 list_0) (x_5 list_0) (x_6 list_0))
	(=>	(and (= x_5 x_6)
			(qrev_0 x_6 xs_0 (cons_0 z_0 y_0))
			(= x_0 (cons_0 z_0 xs_0)))
		(qrev_0 x_5 x_0 y_0))))
(assert (forall ((x_0 list_0) (y_0 list_0) (x_7 list_0))
	(=>	(and (= x_7 y_0)
			(= x_0 nil_0))
		(qrev_0 x_7 x_0 y_0))))
(declare-fun length_0 (Nat_0 list_0) Bool)
(assert (forall ((x_1 list_0) (y_1 Nat_1) (xs_1 list_0) (x_8 Nat_0) (x_9 Nat_0))
	(=>	(and (= x_8 (S_0 x_9))
			(length_0 x_9 xs_1)
			(= x_1 (cons_0 y_1 xs_1)))
		(length_0 x_8 x_1))))
(assert (forall ((x_1 list_0) (x_10 Nat_0))
	(=>	(and (= x_10 Z_0)
			(= x_1 nil_0))
		(length_0 x_10 x_1))))
(declare-fun x_2 (Nat_0 Nat_0 Nat_0) Bool)
(assert (forall ((x_3 Nat_0) (y_2 Nat_0) (z_1 Nat_0) (x_11 Nat_0) (x_12 Nat_0))
	(=>	(and (= x_11 (S_0 x_12))
			(x_2 x_12 z_1 y_2)
			(= x_3 (S_0 z_1)))
		(x_2 x_11 x_3 y_2))))
(assert (forall ((x_3 Nat_0) (y_2 Nat_0) (x_13 Nat_0))
	(=>	(and (= x_13 y_2)
			(= x_3 Z_0))
		(x_2 x_13 x_3 y_2))))
(assert (forall ((x_4 list_0) (y_3 list_0) (x_14 list_0) (x_15 Nat_0) (x_16 Nat_0) (x_17 Nat_0) (x_18 Nat_0))
	(=>	(and (diseqNat_1 x_15 x_18)
			(qrev_0 x_14 x_4 y_3)
			(length_0 x_15 x_14)
			(length_0 x_16 x_4)
			(length_0 x_17 y_3)
			(x_2 x_18 x_16 x_17))
		false)))
(check-sat)
