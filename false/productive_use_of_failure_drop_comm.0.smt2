(set-logic HORN)
(declare-datatypes ((Nat_1 0)) (((Z_1 ) (S_1 (unS_0 Nat_1)))))
(declare-fun diseqNat_0 (Nat_1 Nat_1) Bool)
(declare-fun unS_1 (Nat_1 Nat_1) Bool)
(declare-fun isZ_0 (Nat_1) Bool)
(declare-fun isS_0 (Nat_1) Bool)
(assert (forall ((x_14 Nat_1) (x_13 Nat_1))
	(=> (= x_14 (S_1 x_13))
	    (unS_1 x_13 x_14))))
(assert (isZ_0 Z_1))
(assert (forall ((x_15 Nat_1))
	(isS_0 (S_1 x_15))))
(assert (forall ((x_16 Nat_1))
	(diseqNat_0 Z_1 (S_1 x_16))))
(assert (forall ((x_17 Nat_1))
	(diseqNat_0 (S_1 x_17) Z_1)))
(assert (forall ((x_18 Nat_1) (x_19 Nat_1))
	(=> (diseqNat_0 x_18 x_19)
	    (diseqNat_0 (S_1 x_18) (S_1 x_19)))))
(declare-fun add_0 (Nat_1 Nat_1 Nat_1) Bool)
(declare-fun minus_0 (Nat_1 Nat_1 Nat_1) Bool)
(declare-fun le_0 (Nat_1 Nat_1) Bool)
(declare-fun ge_0 (Nat_1 Nat_1) Bool)
(declare-fun lt_0 (Nat_1 Nat_1) Bool)
(declare-fun gt_0 (Nat_1 Nat_1) Bool)
(declare-fun mult_0 (Nat_1 Nat_1 Nat_1) Bool)
(declare-fun div_0 (Nat_1 Nat_1 Nat_1) Bool)
(declare-fun mod_0 (Nat_1 Nat_1 Nat_1) Bool)
(assert (forall ((y_1 Nat_1))
	(add_0 y_1 Z_1 y_1)))
(assert (forall ((x_11 Nat_1) (y_1 Nat_1) (r_0 Nat_1))
	(=> (add_0 r_0 x_11 y_1)
	    (add_0 (S_1 r_0) (S_1 x_11) y_1))))
(assert (forall ((y_1 Nat_1))
	(minus_0 Z_1 Z_1 y_1)))
(assert (forall ((x_11 Nat_1) (y_1 Nat_1) (r_0 Nat_1))
	(=> (minus_0 r_0 x_11 y_1)
	    (minus_0 (S_1 r_0) (S_1 x_11) y_1))))
(assert (forall ((y_1 Nat_1))
	(le_0 Z_1 y_1)))
(assert (forall ((x_11 Nat_1) (y_1 Nat_1))
	(=> (le_0 x_11 y_1)
	    (le_0 (S_1 x_11) (S_1 y_1)))))
(assert (forall ((y_1 Nat_1))
	(ge_0 y_1 Z_1)))
(assert (forall ((x_11 Nat_1) (y_1 Nat_1))
	(=> (ge_0 x_11 y_1)
	    (ge_0 (S_1 x_11) (S_1 y_1)))))
(assert (forall ((y_1 Nat_1))
	(lt_0 Z_1 (S_1 y_1))))
(assert (forall ((x_11 Nat_1) (y_1 Nat_1))
	(=> (lt_0 x_11 y_1)
	    (lt_0 (S_1 x_11) (S_1 y_1)))))
(assert (forall ((y_1 Nat_1))
	(gt_0 (S_1 y_1) Z_1)))
(assert (forall ((x_11 Nat_1) (y_1 Nat_1))
	(=> (gt_0 x_11 y_1)
	    (gt_0 (S_1 x_11) (S_1 y_1)))))
(assert (forall ((y_1 Nat_1))
	(mult_0 Z_1 Z_1 y_1)))
(assert (forall ((x_11 Nat_1) (y_1 Nat_1) (r_0 Nat_1) (z_1 Nat_1))
	(=>	(and (mult_0 r_0 x_11 y_1)
			(add_0 z_1 r_0 y_1))
		(mult_0 z_1 (S_1 x_11) y_1))))
(assert (forall ((x_11 Nat_1) (y_1 Nat_1))
	(=> (lt_0 x_11 y_1)
	    (div_0 Z_1 x_11 y_1))))
(assert (forall ((x_11 Nat_1) (y_1 Nat_1) (r_0 Nat_1) (z_1 Nat_1))
	(=>	(and (ge_0 x_11 y_1)
			(minus_0 z_1 x_11 y_1)
			(div_0 r_0 z_1 y_1))
		(div_0 (S_1 r_0) x_11 y_1))))
(assert (forall ((x_11 Nat_1) (y_1 Nat_1))
	(=> (lt_0 x_11 y_1)
	    (mod_0 x_11 x_11 y_1))))
(assert (forall ((x_11 Nat_1) (y_1 Nat_1) (r_0 Nat_1) (z_1 Nat_1))
	(=>	(and (ge_0 x_11 y_1)
			(minus_0 z_1 x_11 y_1)
			(mod_0 r_0 z_1 y_1))
		(mod_0 r_0 x_11 y_1))))
(declare-datatypes ((Nat_0 0)) (((S_0 (projS_0 Nat_0)) (Z_0 ))))
(declare-fun diseqNat_1 (Nat_0 Nat_0) Bool)
(declare-fun projS_1 (Nat_0 Nat_0) Bool)
(declare-fun isS_1 (Nat_0) Bool)
(declare-fun isZ_1 (Nat_0) Bool)
(assert (forall ((x_21 Nat_0) (x_20 Nat_0))
	(=> (= x_21 (S_0 x_20))
	    (projS_1 x_20 x_21))))
(assert (forall ((x_23 Nat_0))
	(isS_1 (S_0 x_23))))
(assert (isZ_1 Z_0))
(assert (forall ((x_24 Nat_0))
	(diseqNat_1 (S_0 x_24) Z_0)))
(assert (forall ((x_25 Nat_0))
	(diseqNat_1 Z_0 (S_0 x_25))))
(assert (forall ((x_26 Nat_0) (x_27 Nat_0))
	(=> (diseqNat_1 x_26 x_27)
	    (diseqNat_1 (S_0 x_26) (S_0 x_27)))))
(declare-datatypes ((list_0 0)) (((nil_0 ) (cons_0 (head_0 Nat_0) (tail_0 list_0)))))
(declare-fun diseqlist_0 (list_0 list_0) Bool)
(declare-fun head_1 (Nat_0 list_0) Bool)
(declare-fun tail_1 (list_0 list_0) Bool)
(declare-fun isnil_0 (list_0) Bool)
(declare-fun iscons_0 (list_0) Bool)
(assert (forall ((x_31 list_0) (x_29 Nat_0) (x_30 list_0))
	(=> (= x_31 (cons_0 x_29 x_30))
	    (head_1 x_29 x_31))))
(assert (forall ((x_31 list_0) (x_29 Nat_0) (x_30 list_0))
	(=> (= x_31 (cons_0 x_29 x_30))
	    (tail_1 x_30 x_31))))
(assert (isnil_0 nil_0))
(assert (forall ((x_32 Nat_0) (x_33 list_0))
	(iscons_0 (cons_0 x_32 x_33))))
(assert (forall ((x_34 Nat_0) (x_35 list_0))
	(diseqlist_0 nil_0 (cons_0 x_34 x_35))))
(assert (forall ((x_36 Nat_0) (x_37 list_0))
	(diseqlist_0 (cons_0 x_36 x_37) nil_0)))
(assert (forall ((x_38 Nat_0) (x_39 list_0) (x_40 Nat_0) (x_41 list_0))
	(=> (diseqNat_1 x_38 x_40)
	    (diseqlist_0 (cons_0 x_38 x_39) (cons_0 x_40 x_41)))))
(assert (forall ((x_38 Nat_0) (x_39 list_0) (x_40 Nat_0) (x_41 list_0))
	(=> (diseqlist_0 x_39 x_41)
	    (diseqlist_0 (cons_0 x_38 x_39) (cons_0 x_40 x_41)))))
(declare-fun drop_0 (list_0 Nat_0 list_0) Bool)
(assert (forall ((x_0 Nat_0) (y_0 list_0) (x_3 list_0))
	(=>	(and (= x_3 y_0)
			(= x_0 Z_0))
		(drop_0 x_3 x_0 y_0))))
(assert (forall ((x_0 Nat_0) (y_0 list_0) (z_0 Nat_0) (x_1 Nat_0) (x_2 list_0) (x_4 list_0) (x_5 list_0))
	(=>	(and (= x_4 x_5)
			(drop_0 x_5 z_0 x_2)
			(= x_0 (S_0 z_0))
			(= y_0 (cons_0 x_1 x_2)))
		(drop_0 x_4 x_0 y_0))))
(assert (forall ((x_0 Nat_0) (y_0 list_0) (z_0 Nat_0) (x_6 list_0))
	(=>	(and (= x_6 nil_0)
			(= x_0 (S_0 z_0))
			(= y_0 nil_0))
		(drop_0 x_6 x_0 y_0))))
(assert (forall ((n_0 Nat_0) (m_0 Nat_0) (xs_0 list_0) (x_7 list_0) (x_8 list_0) (x_9 list_0) (x_10 list_0))
	(=>	(and (diseqlist_0 x_8 x_10)
			(drop_0 x_7 m_0 xs_0)
			(drop_0 x_8 n_0 x_7)
			(drop_0 x_9 n_0 xs_0)
			(drop_0 x_10 m_0 x_9))
		false)))
(check-sat)
